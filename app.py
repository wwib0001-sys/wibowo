...
def load_data():
    ...
    df['power_pred_base'] = df['predictive_hvac_power_kW'] # Baseline predictive before Grid Event
...
# Get time range specific to the chosen day
...
event_type = st.sidebar.selectbox(
    ...
    options=["Contingency FCAS", "Peak Demand / RERT"]
)
...
def apply_grid_override(df_day_subset):
    ...
    # Recalculate Energy based on the dynamic power
    df_dyn['energy_interval_pred'] = df_dyn['power_pred'] * (5/60) # Assuming 5 min intervals
    return df_dyn

# Filter to live time and apply logic
df_live = df_day[df_day.index.time <= current_time]
df_live_dyn = apply_grid_override(df_live)

# Calculate Cumulative Energies for the day up to the current time
daily_energy_true = df_live_dyn['energy_interval_true'].sum()
daily_energy_pred = df_live_dyn['energy_interval_pred'].sum()
...
def calc_live_metrics(data):
    ...
    # Recalculate predictive comfort against the dynamic temperature
